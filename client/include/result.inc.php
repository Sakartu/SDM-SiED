<?php
    ...
    require_once(dirname(__FILE__).'/../include/result_process.inc.php');
            ...
            function vNodeId($resultNum, $pre, $cid)
            {
                return "v_".$resultNum."_".$pre."_".$cid;
            }

            function tNodeId($resultNum, $pre, $cid)
            {
                return "t_".$resultNum."_".$pre."_".$cid;
            }

            function aNodeId($resultNum, $pre, $cid)
            {
                return "a_".$resultNum."_".$pre."_".$cid;
            }
                ...
                foreach ($result['rowArrays'] as $rowArrayIndex => $rowArray)
                {
                    ...
                    if (!empty($nodeRowObjects))
                    {
                        ...
                        while(!empty($queue))
                        {
                            ...
                            echo $spacing.'&lt;<span class="nodeElem" id="'.tNodeId($rowArrayIndex, $nodeRow->pre, $cid).'">'.$nodeRow->tag.'</span>';
                            ...
                            while(!empty($queue) && array_peek($queue)->isAttrNode())
                            {
                                ...
                                echo ' <span class="nodeElem" id="'.aNodeId($rowArrayIndex, $attrNodeRow->pre, $cid).'">'.$attTag.'</span>'.
                                '=<span class="nodeElem textElem" id="'.vNodeId($rowArrayIndex, $attrTextRow->pre, $cid).'">"'.$attrTextRow->val.'"</span>';
                                $expectedPost++;
                                $expectedPost++;
                            }
                            ...
                            else 
                            {
                                ...
                                while(!empty($queue) && array_peek($queue)->isTextNode())
                                {
                                    ...
                                    echo $spacing.'<span class="nodeElem textElem" id="'.vNodeId($rowArrayIndex, $textNodeRow->pre, $cid).'">'.$textNodeRow->val.'</span><br/>'."\n";
                                    $expectedPost++;
                                }
                            }
                            ...
                            while(!empty($stack))
                            {
                                ...
                                if ($closeNode->post == $expectedPost)
                                {
                                    ...
                                    echo $spacing.'&lt;/<span class="nodeElem" id="'.tNodeId($rowArrayIndex, $closeNode->pre, $cid).'">'.$closeNode->tag.'</span>&gt;<br/>'."\n";    
                                }
                                else 
                                {
                                    break;
    ...
    $(".nodeElem").click(function(){
        ...
        jPrompt('Enter new '+strName+':', '', 'Prompt Dialog', function(r) {
            ...
            if( txt != "")
            {
                ...
                window.location.href= window.location.href+"&action=update&type="+urlencode(type)+"&pre="+urlencode(pre)+"&txt="+urlencode(txt)+"&cid="+urlencode(cid);
            }
        });
    })
</script>